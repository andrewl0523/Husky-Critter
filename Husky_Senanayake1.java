...
public class Husky_Senanayake1 extends Critter {
	private boolean hasMated = false;
	public boolean isMating = false;
	private boolean isSleeping = false;
	private int turnCount = 0;
	private static final int MOVESTOEVADE = 50;
	private static final int MOVESTOMATE = 400;
	...
	private static ArrayList<Husky_Senanayake1> huskies = new ArrayList<Husky_Senanayake1>();
	private static int winCountToEnemy;
	private static int loseCountToEnemy;
	public Husky_Senanayake1() {
		huskies.add(this);
	}
	@Override
	public void sleep() {
	...
	{
		for (Husky_Senanayake1 husky : huskies)
		{
	...
	public Direction getMove() {
		turnCount++;
		if(turnCount < MOVESTOMATE || loseCountToEnemy > 15)
		{
			for (Direction direction : Direction.values())
			{
				if (direction == Direction.CENTER)
				...
				if (!hasMated && turnCount > MOVESTOEVADE && turnCount < MOVESTOMATE)
				{
					int x = getX();
					int y = getY();
				...
				else if (turnCount < MOVESTOEVADE)
				{
